...
# set db name
DB_NAME = "ZET_DNXPool.db"

# connect to database
conn = sqlite3.connect(db_path+"/"+DB_NAME, timeout = 30.0)
cur = conn.cursor() 
...
class ZET_DNX:
    def __init__(self):
        self.api_url = 'https://zetpool.org/dnx'
        self.headers = {
    ...
    def getBlocksData(self):
        url = self.api_url + '/api/stats'
        try:
            r = self.session.get(url, timeout = 5)
            data = r.json()
            # latest_matured = data['pool']
...
class ZET_DNXPool(commands.Cog):
    ...
    async def on_ready(self):
        print("ZET_DNXPool.py is ready")

    
    # # add wallet to wallet_watch   
    # @commands.command() 
    # async def add_watch_wallet_dnx(self, ctx, wallet, description = "add wallet to watch list to get notifications, usage example: !add_watch_wallet_dnx XwoFTZ33xo7L43w1n1Bsj5NNtGcM8McgpBLchScS5CLqRDAcdMi3DiYK5DvthVnnuyQy7qLegkULuXAYs3jDzZYT2meoPpjUs"):
    #     # get user id
    ...
    # @commands.command() 
    # async def remove_watch_wallet_dnx(self, ctx, description = "remove wallet from watch list and stop getting notifications, usage example: !remove_watch_wallet_dnx"):
    #     # get user id
    #     author_id = str(ctx.author.id)
    ...
    async def new_block_check(self):
        ...
        if pool_blocks_data != None:
            ...
            if int(height) > int(cached_height):
                # create embed
                embed_message = discord.Embed(title = f"NEW BLOCK!", description = f"For DNX ZETpool", color = discord.Color.green(), url = f"https://zetpool.org/dnx/#pool_blocks")
...
async def setup(bot):
    await bot.add_cog(ZET_DNXPool(bot))
